...
class PatternTestWrapper {
 ...
 public:
  ...
  void shouldReturn(
      std::vector<std::pair<arangodb::aql::ExecutionState,
                            arangodb::aql::SharedAqlItemBlockPtr>>& toReturn) {
  ...
  virtual void PullAndAssertDataRows(
      std::vector<std::string> const& dataResults) = 0;

  /// Helper method to assert that we can now pull all ShadowRows at once, and
  /// stop at the next data row Also asserts that we will never leave the
  /// finalState (either HASMORE or DONE)
  void PullAndAssertShadowRows(
...
CutMyBlockIntoPieces(SharedAqlItemBlockPtr baseBlock, uint64_t piecesBitMap) {
  ...
  for (size_t to = 0; to < baseBlock->size(); ++to) {
    if (((piecesBitMap) >> (to)) & 1) {
      ...
      ExecutionState state = to == baseBlock->size() - 1
                                 ? ExecutionState::DONE
                                 : ExecutionState::HASMORE;
      toReturn.emplace_back(
          std::make_pair(state, baseBlock->slice(from, to + 1)));
      from = to + 1;
    }
  }
  if (from < baseBlock->size()) {
    toReturn.emplace_back(std::make_pair(
        ExecutionState::DONE, baseBlock->slice(from, baseBlock->size())));
  }
  return toReturn;
}

// Section First Pattern, alternating Input / Shadow Rows, 1 higher level shadow
// row
#define TEST_SHADOWROW_PATTERN_1(FetcherWrapper, TestName)                    \
  ...
  TEST_P(TestName, handle_shadow_rows) {                                      \
    ...
    wrapper.PullAndAssertShadowRows({{0, "c"}, {1, "c"}},                     \
                                    ExecutionState::DONE);                    \
    /* Done check */                                                          \
  ...
  INSTANTIATE_TEST_CASE_P(                                                    \
      TestName##Instanciated, TestName,                                       \
      testing::Range(static_cast<uint64_t>(0),                                \
                     static_cast<uint64_t>(std::pow(2, 6))));

// Section Second Pattern, two consecutive relevant shadow rows, , 1 higher
// level shadow row
#define TEST_SHADOWROW_PATTERN_2(FetcherWrapper, TestName)                    \
  ...
  TEST_P(TestName, handle_shadow_rows) {                                      \
    ...
    wrapper.PullAndAssertShadowRows({{0, "b"}, {1, "b"}},                     \
                                    ExecutionState::DONE);                    \
    wrapper.StaysConstantAfterDone();                                         \
  }                                                                           \
                                                                              \
...
#define TEST_SHADOWROW_PATTERN_3(FetcherWrapper, TestName)                    \
  ...
  TEST_P(TestName, handle_shadow_rows) {                                      \
    ...
    wrapper.PullAndAssertDataRows({"a"});                                     \
                                                                              \
    /* Fetch shadow rows */                                                   \
    wrapper.PullAndAssertShadowRows({{0, "a"}, {1, "a"}},                     \
                                    ExecutionState::HASMORE);                 \
                                                                              \
...
#define TEST_SHADOWROW_PATTERN_5(FetcherWrapper, TestName)                    \
  ...
  TEST_P(TestName, handle_shadow_rows) {                                      \
    ...
    wrapper.PullAndAssertShadowRows({{0, "h"}, {1, "i"}, {2, "j"}},           \
                                    ExecutionState::DONE);                    \
    wrapper.StaysConstantAfterDone();                                         \
  }                                                                           \
                                                                              \
  INSTANTIATE_TEST_CASE_P(                                                    \
      TestName##Instanciation, TestName,                                      \
      testing::Range(static_cast<uint64_t>(0),                                \
